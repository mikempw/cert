...
export default function CertificateWizard({ mode = 'issue' }) {
  ...
  const answerQuestion = async (question, value) => {
    ...
    const requestBody = {
      ...
      question_id: question,  // Note: 'question_id' is the key, 'question' is the variable
      value: value           // 'value' is both the key and the variable
    }

    console.log('Sending answer:', JSON.stringify(requestBody, null, 2))
    ...
    try {
      switch(stepId) {
        ...
        case 'provider':
          ...
          await answerQuestion('challenge_type', 'http-01')
        ...
        case 'bigip':
          ...
          await answerQuestion('clientssl_profile', data.profile || '')
          await answerQuestion('virtual_server', data.vs || '')
          break
        case 'vault':
          await answerQuestion('key_secret_path', data)
          break
      }
  ...
  try {
    ...
    console.log('Committing with:', requestBody)
    console.log('Current slots:', slots)
    
    const response = await fetch('/api/guided/commit', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
